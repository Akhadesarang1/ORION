...
class IntentClassifier:
    ...
    def classify(self, text: str) -> dict:
        ...
        if is_create and is_doc:
            ...
            doc_kw = "document"
            for k in [
                "documentation", "document", "presentation", "report",
                "paper", "analysis", "slide deck", "slides",
                "summary", "brief", "overview"
            ]:
                if k in t:
                    doc_kw = k
            ...
            topic = re.sub(
                r"^(of|about|on|covering|titled|regarding)\b\s*", "", topic_raw
            ).strip()
            topic = re.sub(
                r"\b(in|format|as)\s+(docx|doc|pdf|txt|md|ppt|pptx)\b.*", "", topic
            ).strip()

            # Clean mode words from topic
            topic = re.sub(
                r"\b(deep|detailed|brief|short|comprehensive)\b",
        ...
        # ================= CODE GENERATION =================
        if re.search(
            r"\b(write|generate|create|code|script)\b.*\b(python|script|program|app|game|tool)\b",
            t
        ):
            # Extract topic
            topic = raw
            prefixes = [
                ...
                "code a", "python script for", "python program for"
            ]
            for garbage in prefixes:
        ...
        # ================= CONFIRM =================
        confirm_words = (
            "confirm", "yes", "approve", "proceed", "y", "ok", "okay",
            "confrim", "kill it", "act", "do it", "execute"
        )
        if clean_t in confirm_words:
        ...
        if m:
            ...
            # Name-based (might fail safely)
            return {"intent": "DEFENSE_KILL", "target": target}
