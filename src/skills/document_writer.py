...
class DocumentWriter:
    ...
    def write_docx(path: str, content: str):
        ...
        for line in lines:
            ...
            if line.startswith("#"):
                ...
                # Strip internal markdown (** or __) from headers for clean
                # look
                text = text.replace("**", "").replace("__", "")

                style = (
                    "ORION_Title" if level == 1
                    else f"ORION_Heading{level}"
                )
    ...
    def _configure_styles(doc: Document):
        ...
        def safe_style(name, style_type):
            return styles[name] if name in styles else styles.add_style(
                name, style_type)
