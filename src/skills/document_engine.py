...
class DocumentEngine:
    ...
    def generate_slide_content(self, topic, section, context=None):
        prompt = (
            f"Create 3-5 high-impact bullet points for a \n"
            f"presentation slide about '{section}'.\n"
            f"Topic: {topic}\n"
            "Format: Plain text, one bullet per line. No markdown symbols "
            "like '*' or '-'.\n"
    ...
    def _append_to_output(self, job, section, content):
        if job.output_format == "pptx":
            ...
            if not hasattr(job, 'slide_map'):
                job.slide_map = {}

            # Clean bullets
            bullets = [line.strip().lstrip("-*• ")
    ...
    def generate_outline(
        ...
        if pages:
            ...
            prompt += (
                f"Create a Detailed Linear Outline for a {pages}-page "
                f"document on '{topic}'.\n"
                f"Target Section Count: {target_sections} sections.\n"
                "Format: simple numbered list (1. ...).\n"
                "Ensure logical flow from Introduction to "
                "Advanced Analysis to Conclusion."
            )
        elif mode == "slide":
            prompt += (
                f"Act as a senior academic presentation strategist. Create "
                f"a structure for a presentation on '{topic}'.\n"
                ...
                "10. Conclusion\n"
                "11. Key Takeaway\n"
                "Do not add sub-bullets or explanations. Just the 11 "
                "slide titles numbered 1-11."
            )
        elif mode == "deep":
            prompt += (
                f"Create a COMPREHENSIVE, DEEP-DIVE table of contents "
                f"(20+ sections) for the topic '{topic}'.\n"
                "The outline must be exhaustively detailed.\n"
                "If the topic is Technical (software/engineering):\n"
                "  Include: System Architecture, Data Flow, Security, "
                "Error Handling, Scalability, Deployment, Trade-offs.\n"
                "If the topic is General/Historical (e.g. History, Science):\n"
                "  Include: Origins, Timeline, Key Figures, Socio-Economic "
                "Impact, Cultural Legacy, Modern Interpretations.\n"
                "Format: simple numbered list (1. ...).\n"
            )
        elif mode == "brief":
            prompt += (
                f"Create a short, executive summary outline (3-5 sections) "
                f"for '{topic}'.\n"
            ...
            prompt += (
                ...
                "Structure: Logical flow suitable for the topic "
                "(Introduction -> Body -> Conclusion)."
            )

        prompt += (
            "\n\nCRITICAL DIRECTIVE: You are an objective, academic "
            "historian and technical writer. "
    ...
    def expand_section(self, topic, section, context=None, mode="standard"):
        ...
        if mode == "slide":
            prompt += (
                ...
                "3. Do NOT use Markdown formatting (no **bold**, no "
                "## headings, no ===).\n"
                "4. Do NOT repeat the slide title.\n"
                "5. Write in incomplete sentences (slide style), not "
                "paragraphs.\n"
                "6. Focus on key facts, data, or insights.\n"
            )
        else:
            prompt += (
                f"Write the '{section}' section for the technical/academic "
                f"documentation of '{topic}'.\n"
        ...
        if mode == "slide":
            ...
            prompt += "Explain what this module does in simple terms. " \
                      "High-level summary."
            prompt += "Explain WHY this exists. " \
                      "What problem does it solve?"
            prompt += "Describe the high-level structure. How do the " \
                      "parts connect? Use text diagrams if helpful."
            prompt += "List key classes/functions and their specific " \
                      "responsibilities."
        elif "Workflow" in section:
            prompt += "Step-by-step execution flow. Input -> Process -> Output."
            prompt += "How is state/data stored? JSON structure? Database " \
                      "schema? Memory?"
        elif "Security" in section:
            prompt += "Detail permissions, validation checks, " \
                      "and safeguards."
            prompt += "How are exceptions handled? What are the " \
                      "current constraints?"
    ...
    def start_job(
        ...
        for line in outline_text.splitlines():
            ...
            if line.startswith('[') and line.endswith(']'):
                try:
                    ...
                    if isinstance(parsed_list, list):
                        sections.extend([
                            str(item).strip()
                            for item in parsed_list if str(item).strip()
                        ])
            ...
            if line[0].isdigit() or line.startswith(
                    '-') or line.startswith('*'):
                # Strip out the leading bullet/number for a cleaner section
                # title
                import re
        ...
        if not sections:
            print(
                f"[ORION DOC] ⚠️ Outline generation failed for mode '{mode}'. "
                f"Retrying with standard mode..."
            )
            outline_text = self.generate_outline(topic, context, "standard")
            print(
                f"[ORION DOC] Raw LLM Outline Output (Retry):\n"
                f"{outline_text}\n"
            )
            for line in outline_text.splitlines():
        ...
        if not sections:
            print(
                "[ORION DOC] ⚠️ Outline generation failed completely. "
    ...
    def process_job(self):
        ...
        try:
            ...
            while job.current_index < total:
                ...
                # formatted_content was old logic. Now we pass raw content +
                # section
                ...
                # 5. ETA Calculation
                avg_time = elapsed if job.current_index == 1 else (
                    elapsed + (getattr(job, 'avg_section_time', elapsed))) / 2
